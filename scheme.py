...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        # code to evaluate call expressions
        # evaluate the opperator by lookup - excepting an error
        procedure = scheme_eval(first,env)
        # if no error is raised check procedure is indeed a procedure
        check_procedure(procedure)
        # call eval_call on procedure with the rest of the exp as arguments
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    # if expression is nil return None
    if expressions == nil:
        return None
    elif expressions.second == nil:
        # recursive base case - last expression is reached
        return scheme_eval(expressions.first, env)
    else:
        # evaluate expressions even though they are not returned (to bind vairables etc)
        scheme_eval(expressions.first, env)
        # recurse to reach last expression
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 3
        # take a symbol and bind it to a value in the current frame
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        # check for binding in current frame (self.bindings), if not found try parents
        try:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        # raise SchemeError if number of argument values does not match number of formal parameters
        if len(formals) > len(vals):
            raise SchemeError('Too Few Values Given')
        elif len(formals) < len(vals):
            raise SchemeError('Too Many Values Given')
        else:
            # corect number of values therfore set bindings - child.bindings = {}
            # arbitrary which sets while condition
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # the function we use is defined within each pair
        # create a lambda function to deal with environment
        evaluated_ops = operands.map(lambda x: scheme_eval(x,env))
        return scheme_apply(self,evaluated_ops,env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while args is not nil:
            ...
            args = args.second
        # BEGIN PROBLEM 4
        # check if self.use_env is True, if so pass environment in as last argument
        if self.use_env:
            python_args.append(env)
        # call self.fn on the arguments - if error of TypeError occurs, convert to SchemeError
        try:
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        # create a new Frame instance from parent frame (WHERE LAMBDA FUNC IS DEFINED - self.env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        # bind name to value in the given envirnoment
        ...
        # BEGIN PROBLEM 10
        # bind symbol (target.first) to user defined procedure
        # procedure is evaluated as a lambda function
        symbol = target.first
        # make a new pair, striping out the symbol and leaving formal parameters and function body
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    # simply return the opperand expression
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    # function creates LambdaProcedure instances
    # the body attribute is a scheme list of body expressions
    # the formals attribute is also a scheme list of formal parameters
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    # if there are no subexpressions then return True
    if expressions == nil:
        return True
    # otherwise evaluate the current sub-expression
    evaluated_expression = scheme_eval(expressions.first,env)
    # handle case in which we have last sub-expression which is true
    if scheme_truep(evaluated_expression):
        if expressions.second == nil:
            return evaluated_expression
        else:
        # in this case the sub-expression is true but we need to continue to evaluate the expression list
            return do_and_form(expressions.second,env)
    else:
        # this is the case of a falsey value, in which case return False
        return False
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    # if there are no subexpressions then return False
    if expressions == nil:
        return False
    ...
    # if false value has been encountered move to next sub-expression
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            # if there are no subexpressions true the predicate value
            if clause.second is nil:
                return test
            # evaluate all subexpressions and return the last one (if subexpressions exist)
            else:
                return eval_all(clause.second,env) # default behaviour is to return last expression
            # END PROBLEM 14
        expressions = expressions.second
        # there are no true predicates and no else - return None
    return None
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    # intialise child frame
    let_frame  = env.make_child_frame(nil,nil)
    # iterate through each binding
    while bindings is not nil:
        # isolate first binding
        binding = bindings.first
        # check form of the binding
        check_form(binding,2,2)
        # check formal param is of correct form
        formal = binding.first
        check_formals(Pair(formal,nil))
        # Evaluate value in parent frame (ENV)
        value = scheme_eval(binding.second.first,env)
        # bind formal params to evaluated values inside the child let frame
        let_frame.define(formal,value)
...
class MuProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # create a new Frame instance from parent frame (where mu function is called - env)
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    # function creates MuProcedure instances
    # the body attribute is a scheme list of body expressions
    # the formals attribute is also a scheme list of formal parameters
